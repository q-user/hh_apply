"""CLI-операция ``channel-monitor`` (VSA rewrite, issue #147)."""
...
class _ChannelMonitorSlice(Protocol):
    """Minimal slice contract for ``channel-monitor``."""
...
class Operation(BaseOperation):
    ...
    def __init__(self, slice_: _ChannelMonitorSlice | None = None) -> None:
        self._slice = slice_
    ...
    def run(self, args: argparse.Namespace) -> int:
        if self._slice is None:
            logger.error("channel-monitor requires a channel_monitoring slice")
            return 1
        slice_ = self._slice
    ...
    def _print_help() -> None:
        print(
            ...
            "  channel-monitor --add --name <NAME> --channel-id <ID> "
            "[--keywords k1,k2]\n"
