"""HH.ru API client exceptions (VSA port of
``hh_applicant_tool.api.errors``, issue #152).

The shared kernel owns the *generic* exception hierarchy
(:class:`BadResponse`, :class:`ApiError`, :class:`Redirect`,
:class:`ClientError`, :class:`BadRequest`, :class:`Forbidden`,
:class:`ResourceNotFound`, :class:`InternalServerError`,
:class:`BadGateway`). Slice-specific exceptions such as
:class:`CaptchaRequired` and :class:`LimitExceeded` live in the
:mod:`job_bot.application_submit.errors` module alongside
:class:`RetryableError` / :class:`FatalError`.
"""

from __future__ import annotations

from typing import Any, Type

from requests import Request, Response
from requests.adapters import CaseInsensitiveDict

__all__ = (
    "BadResponse",
    "ApiError",
    "BadGateway",
    "BadRequest",
    "ClientError",
    "Forbidden",
    "InternalServerError",
    "Redirect",
    "ResourceNotFound",
    "has_error_value",
)


class BadResponse(Exception):
    pass


def has_error_value(value: str, data: dict[str, Any]) -> bool:
    """Return ``True`` if any entry in ``data['errors']`` carries ``value``."""
    return any(v.get("value") == value for v in data.get("errors", []))


class ApiError(BadResponse):
    def __init__(self, response: Response, data: dict[str, Any]) -> None:
        self._response = response
        self._data = data

    @property
    def data(self) -> dict[str, Any]:
        return self._data

    @property
    def request(self) -> Request:
        # ``requests.Response.request`` is typed as ``PreparedRequest | None``;
        # we mirror the legacy behaviour of returning it as ``Request``.
        request = self._response.request
        assert request is not None
        return request  # type: ignore[return-value]

    @property
    def status_code(self) -> int:
        return self._response.status_code

    @property
    def response_headers(self) -> CaseInsensitiveDict[str, str]:  # type: ignore[type-arg]
        return self._response.headers

    @property
    def message(self) -> str:
        return (
            self._data.get("error_description")
            or self._data.get("description")
            or (
                "An errors has occurred: "
                + "; ".join(
                    v["type"] + (f": {v['value']}" if "value" in v else "")
                    for v in self._data["errors"]
                )
            )
            if "errors" in self._data
            else str(self._data)
        )

    #     def __getattr__(self, name: str) -> Any:
    #         try:
    #             return self._raw[name]
    #         except KeyError as ex:
    #             raise AttributeError(name) from ex

    def __str__(self) -> str:
        return self.message

    @staticmethod
    def has_error_value(value: str, data: dict[str, Any]) -> bool:
        return any(v.get("value") == value for v in data.get("errors", []))

    @classmethod
    def raise_for_status(
        cls: Type[ApiError], response: Response, data: dict[str, Any]
    ) -> None:
        # Lazy import to break the dependency cycle: the slice-specific
        # exception classes live in :mod:`job_bot.application_submit.errors`
        # and that module re-exports :class:`ClientError` from here.
        from job_bot.application_submit.errors import (
            CaptchaRequired,
            LimitExceeded,
        )

        match response.status_code:
            case status if 300 <= status <= 308:
                raise Redirect(response, data)
            case 400:
                if cls.has_error_value("limit_exceeded", data):
                    raise LimitExceeded(response, data)
                raise BadRequest(response, data)
            case 403:
                if cls.has_error_value("captcha_required", data):
                    raise CaptchaRequired(response, data)
                raise Forbidden(response, data)
            case 404:
                raise ResourceNotFound(response, data)
            case status if 500 > status >= 400:
                raise ClientError(response, data)
            case 502:
                raise BadGateway(response, data)
            case status if status >= 500:
                raise InternalServerError(response, data)


class Redirect(ApiError):
    pass


class ClientError(ApiError):
    pass


class BadRequest(ClientError):
    pass


class Forbidden(ClientError):
    pass


class ResourceNotFound(ClientError):
    pass


class InternalServerError(ApiError):
    pass


# По всей видимости, прокси возвращает, когда их бекенд на Java падает
# {'description': 'Bad Gateway', 'errors': [{'type': 'bad_gateway'}], 'request_id': '<md5 хеш>'}
class BadGateway(InternalServerError):
    pass
