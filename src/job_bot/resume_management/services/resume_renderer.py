"""Resume markdown -> POST /resumes payload renderer (issue #151).

VSA port of the legacy :mod:`hh_applicant_tool.utils.resume_md`
module. The :class:`ResumeRenderer` service exposes a single
:meth:`ResumeRenderer.render` method that turns a markdown resume
template into the dict payload expected by ``POST /resumes``.

The canonical VSA surface is the :class:`ResumeRenderer` class; the
module-level :func:`parse_resume_md` alias is preserved for
backwards compatibility with the
``FileSystemTemplateLoader`` in
:mod:`job_bot.resume_management.handlers.resume_create_handler`.

The renderer is deliberately side-effect free and dependency-free
(only :mod:`re` from stdlib): every lookup table and helper is
defined in this module, no third-party markdown library is used.
"""
...
class ResumeRenderer:
    """Render a markdown resume template to a ``POST /resumes`` payload.

    The renderer is stateless and side-effect free. Tests can use a
    single instance for many ``render`` calls.

    Public surface:

    * :meth:`render` — turn a markdown string into the API dict.
    """
...
def parse_resume_md(text: str) -> dict[str, Any]:
    """Parse a markdown resume template (module-level alias).

    Thin wrapper around :meth:`ResumeRenderer.render`. Kept so
    ``FileSystemTemplateLoader`` and other VSA code can import a
    function without instantiating the service.
    """
    return ResumeRenderer().render(text)
