"""Default apply-one draft implementation (issue #10).

.. versionchanged:: 2.0
   Moved from ``hh_applicant_tool.services.apply_one`` to
   ``job_bot.application_submit.services.apply_one_service``
   as part of the VSA switchover (issue #77).

``make_default_apply_one`` принимает ``api_client`` и возвращает callable,
удовлетворяющий :class:`ApplyOneDraftFn` (контракт воркера).
"""
...
if TYPE_CHECKING:
    from hh_applicant_tool.services.apply_worker import ApplyOneDraftFn
...
def make_default_apply_one(
    ...
    *,
    ...
    from hh_applicant_tool.services.vacancy_tests import VacancyTestsService
