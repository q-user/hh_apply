"""Интерактивный пошаговый ревью черновиков откликов в Telegram (issue #9).

.. versionchanged:: 2.0
   Moved from :mod:`hh_applicant_tool.services.review_flow` (issue #77).
   The legacy module is kept as a deprecation shim that subclasses
   :class:`ReviewFlowService` and emits ``DeprecationWarning`` on
   instantiation. New code should depend on this VSA path directly.

Сервис реализует FSM (9 состояний), которая обрабатывает Telegram-апдейты
(текст + inline-кнопки) и возвращает :class:`OutgoingMessage` для отправки.
...
Зависимости (DI): ``storage``, ``transport`` (для :meth:`send`),
``config``, ``clock`` (по умолчанию ``SystemClock``),
``ai_client`` (опциональный :class:`AIClientPort`).
"""
...
class InlineButton:
    ...
    Attributes:
        text: подпись.
        callback_data: данные для ``callback_query``; для ссылок — :attr:`url`.
        url: если задан, кнопка открывает URL без callback.
    """
...
class OutgoingMessage:
    """Сообщение, которое сервис хочет отправить в Telegram.

    Сервис НЕ отправляет сам — он возвращает список таких DTO, а
    вызывающий код (бот / CLI / тесты) решает, как их доставить.

    Attributes:
        chat_id: идентификатор чата получателя.
        text: текст сообщения.
        reply_markup: двумерный список inline-кнопок.
...
class ReviewFlowService:
    ...
    def process_message(self, update: dict) -> list[OutgoingMessage]:
    ...
    def process_callback(self, update: dict) -> list[OutgoingMessage]:
    ...
    def _load_next_draft(
        ...
        self._bind_session_to_draft(session, next_draft.id, STATE_REVIEW_INTRO)
    ...
    def _intro_continue(
        ...
        if draft is None:
            return []
        if draft.has_test:
            first_answer = self._first_pending_test_answer(draft.id)
    ...
    def send(self, messages: list[OutgoingMessage]) -> list[dict]:
