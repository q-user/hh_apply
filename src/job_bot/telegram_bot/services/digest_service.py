...
.. versionchanged:: 2.0
   Moved from :mod:`hh_applicant_tool.services.daily_digest` (issue #77).
   The legacy module is kept as a deprecation shim that re-exports
   :class:`DailyDigestService` / :class:`DigestResult` / :class:`DraftGroup`
   and the :data:`LAST_DIGEST_KEY` constant. New code should depend on
   ``job_bot.telegram_bot.services.digest_service`` directly.

Сервис собирает статистику из ``application_drafts`` (status='prepared'),
группирует её по ``search_profile_id`` (с разбивкой ``has_test`` /
``без тестов``) и отправляет утреннее сообщение через TelegramTransport.
...
class DailyDigestService:
    ...
    def _resolve_chat_id(self) -> int | None:
        ...
        3. первый ``telegram.allowed_user_ids``.
        """
    ...
    def _generate_ai_summary(
        ...
        if self._ai_client is None or not groups:
            return None
        breakdown = ", ".join(f"{g.profile_name} {g.total}" for g in groups[:5])
        prompt = (
        ...
        except Exception as ex:  # noqa: BLE001 - AI is best-effort; any failure must not block the digest send
